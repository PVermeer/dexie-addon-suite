import { Populate, PopulateOptions, RelationalDbSchema } from '@pvermeer/dexie-populate-addon';
...
export class PopulateTableObservable<T, TKey, B extends boolean, K extends string> extends ObservableTable<T, TKey> {
    ...
    constructor(
        ...
        const _populateResult = async (result: any) => {
                ...
                this[key] = (...args: any[]) => {
                    ...
                    return returnValue.pipe(
                        flatMap(async (result) => {
                            const populated = await _populateResult(result);
                            return { ...populated, result };
                        }),
                        switchMap(({ populated, populatedTree, result }) => this._db.changes$.pipe(
                            filter(changes => changes.some(change => {
                                const obj = 'obj' in change ? change.obj : change.oldObj;
                                return Object.keys(obj).some(objKey => populatedTree[change.table] &&
                                    populatedTree[change.table][objKey] &&
                                    populatedTree[change.table][objKey] === obj[objKey]);
                            })),
                            startWith(null),
                            flatMap(async (_, i) => i === 0 ? populated : _populateResult(result)),
                            distinctUntilChanged(isEqual), share())
                        )
                    );
